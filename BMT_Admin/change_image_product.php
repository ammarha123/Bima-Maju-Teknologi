...
if (isset($_POST['upload'])) {
    ...
    if (!in_array($extension, $allowed_extensions)) {
        ...
        $query = mysqli_query($con, "update main_product SET filename = '$imgnewfile', title = '$title' where id = '$ID'");
        if ($query) {
            echo "<script>alert('Post Edited');</script>";
            echo "<script type='text/javascript'> document.location ='list_main_product.php'; </script>";
        } else {
            echo "Error creating table: " . mysqli_error($con);
                ...
                $ID = $_GET['id'];
                $ret = mysqli_query($con, "select * from main_product where id = '$ID'");
                $cnt = 1;
